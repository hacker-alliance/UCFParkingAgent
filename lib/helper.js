...
module.exports.getTimeCycle = function(time){
  //assume that time is already in EST
  let hour = time.hour();

  if(hour>=5 && hour<=11){
    return flavortext["Time Cycle"][0];

  }else if(hour==12){
    return flavortext["Time Cycle"][1];
  }else if(hour>=13 && hour<=19){
    return flavortext["Time Cycle"][2];
  }else{
    return flavortext["Time Cycle"][3];
  }
}
